...
function generateUniqueFileName(url) {
    ...
    if (url.startsWith('/')) {
        return hash + path_1.default.extname(url);
    }
    let ext = '.jpg';
    try {
        const urlObj = new URL(url);
        const pathname = urlObj.pathname;
        const fileExt = path_1.default.extname(pathname);
        if (fileExt)
            ext = fileExt;
    }
    catch (e) {
...
async function downloadFile(url) {
    ...
    const cachePath = path_1.default.resolve(__dirname, '../public/thumbs/cached', fileName);
    ...
    if (url.startsWith('/')) {
        ...
        const buffer = fs.readFileSync(path_1.default.resolve(__dirname, '../public', url.slice(1)));
        return buffer;
    }
...
function validateSchema(schema, verifyFieldName, data) {
    if (data) {
...
function createPackageFiles(template, viewName) {
    // Read the template, remove the first line if it's an HTML comment, then encode
    const templatePath = path_1.default.resolve(__dirname, `../templates/${template}.liquid`);
    ...
    return [
        ...
        },
        {
            ...
            "Content": readFileAsBase64(path_1.default.resolve(__dirname, `../public/thumbs/${template}-m.png`))
        }
    ];
}
function getWidgetNameFromTemplate(template) {
    const templatePath = path_1.default.resolve(__dirname, `../templates/${template}.liquid`);
...
function createWidgetConfig(template, viewName, schemaDefWidthData, schemaDef) {
    const extendData = {
...
async function createFullPackage(template) {
    console.log(`Creating package for template: ${template}`);
    const viewName = getViewName(template);
    const data = JSON.parse(fs.readFileSync(path_1.default.resolve(__dirname, `../data/${template}.json`), "utf8"));
    const schemaDef = JSON.parse(fs.readFileSync(path_1.default.resolve(__dirname, `../data/${template}.def.json`), "utf8"));
...
async function packWidget(template) {
...
async function packageSingleTemplate(template) {
    ...
    const outputPath = path_1.default.join(outputDir, `${template}.wgt`);
...
async function packageUnpackagedTemplates() {
    ...
    const existingPackages = fs.existsSync(outputDir)
        ...
        : [];
    const unpackagedTemplates = templates.filter(template => !existingPackages.includes(template));
